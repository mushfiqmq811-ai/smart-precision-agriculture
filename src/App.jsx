...
export default function App() {
  ...
  // Calculator State
  const [landSize, setLandSize] = useState(33); // Decimals (শতাংশ)

  // Simulation: Live IoT Data Telemetry
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#070A13] text-slate-100 font-sans flex flex-col selection:bg-emerald-500 selection:text-black print:bg-white print:text-black">
      
      {/* Background Glows */}
      <div className="fixed top-0 left-1/4 w-96 h-96 bg-emerald-600/10 rounded-full blur-[140px] pointer-events-none print:hidden"></div>
      <div className="fixed bottom-0 right-1/4 w-96 h-96 bg-cyan-600/10 rounded-full blur-[140px] pointer-events-none print:hidden"></div>

      {/* Top Navigation */}
      <header className="border-b border-slate-800/80 bg-slate-900/70 backdrop-blur-xl sticky top-0 z-50 print:hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-mono tracking-widest uppercase">PRECISION IOT & RANDOM FOREST ML PLATFORM</p>
            </div>
          </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${
                    activeTab === tab.id
                      ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-950/60 border border-emerald-400/30'
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-8 relative z-10">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-md flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 shadow-2xl">
              <div>
                <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-3">
                ...
                >
                  <span>🖨️</span> {lang === 'bn' ? 'রিপোর্ট প্রিন্ট করুন' : 'Export Report'}
                </button>
                <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              ...
              <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 hover:border-purple-500/50 transition-all shadow-xl backdrop-blur-md">
                ...
                <div className="mt-4">
                  ...
                  <p className="text-xs text-slate-400 mt-2">{lang === 'bn' ? 'পষ্টি উপাদান গ্রহণ ক্ষমতা' : 'Nutrient Availability'}</p>
                </div>
              ...
              <div className="lg:col-span-2 p-6 rounded-3xl bg-slate-900/60 border border-slate-800 shadow-2xl space-y-4 backdrop-blur-md">
                <div className="flex justify-between items-center">
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                    <span>📡</span> {lang === 'bn' ? 'স্যাটেলাইট আবহাওয়া তথ্য' : 'Satellite Meteorological Data'}
                  </h3>
                ...
                <div className="p-4 rounded-2xl bg-teal-950/30 border border-teal-500/30 text-xs text-teal-200 flex items-start space-x-3">
                  ...
                  <div>
                    <strong className="text-teal-300">{lang === 'bn' ? 'এআই কৃষি পরামর্শ:' : 'AI Agronomist Notice:'}</strong> {lang === 'bn' ? 'আজ বাষ্পীভবনের হার কম। ভূগর্ভস্থ পানি ২২% সাশ্রয় করতে সন্ধ্যায় ১২ মিনিটের জন্য ড্রিপ সেচ চলবে।' : 'Low evapotranspiration today. Automated drip valves will run for 12 mins at sunset.'}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500">GPIO 26 • High Voltage Solenoid</p>
                    </div>
                  ...
                  <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 flex justify-between items-center">
                    <div>
                      <p className="text-xs font-bold text-white">{lang === 'bn' ? 'রিলে ২: সার ড্রেসিং ইনজেক্টর' : 'Relay #2: Fertigation Doser'}</p>
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <div className="p-8 rounded-3xl bg-slate-900/80 border border-slate-800 shadow-2xl space-y-6 backdrop-blur-md">
              <div>
                ...
                <h2 className="text-2xl font-black text-white mt-2">{lang === 'bn' ? 'রোগ নির্ণয় ও কৃত্রিম বুদ্ধিমত্তা ল্যাব' : 'AI Pathology & Plant Vision Diagnostics'}</h2>
                <p className="text-xs text-slate-400 mt-1">{lang === 'bn' ? 'আক্রান্ত পাতার ছবি আপলোড করে র‍্যান্ডম ফরেস্ট অ্যান্ড সিএনএন মডেল চালান।' : 'Upload crop leaf image to trigger Random Forest ML classification.'}</p>
              </div>
                ...
                <div className="border-2 border-dashed border-slate-700 hover:border-emerald-500/60 rounded-3xl p-6 text-center bg-slate-950/60 transition-all flex flex-col items-center justify-center min-h-[220px]">
                  {selectedImage ? (
                    <div className="relative w-full h-48 rounded-2xl overflow-hidden border border-slate-800">
                      ...
                      <button onClick={() => setSelectedImage(null)} className="absolute to            
